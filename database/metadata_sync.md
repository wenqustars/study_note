...
export GOPATH=/home/acme/go
export PATH=$PATH:${GOPATH}/bin
#下载工具源码
cd /home/acme/go/src
git clone git@github.com:gangxueZhang/mysql-schema-sync.git
#修改main.go文件
cd mysql-schema-sync
vim main.go
#修改如下
"github.com/hidu/mysql-schema-sync/internal" -> "mysql-schema-sync/internal"
#修改Godeps.json文件
cd mysql-schema-sync
vim Godeps/Godeps.json
#修改如下
"ImportPath": "github.com/hidu/mysql-schema-sync", -> "ImportPath": "mysql-schema-sync",
#安装应用
cd ..
go install mysql-schema-sync
#安装完成：命令文件在/home/acme/go/bin目录下
```
...
{
      //source：同步源
      "source":"user:password@(127.0.0.1:3306)/test_0",
      //dest：待同步的数据库
      "dest":"user:password@(127.0.0.1:3306)/test_1",
...
source: 数据库同步源  
dest:   待同步的数据库  
tables： 数组，配置需要同步的表，为空则是不限制，例如: ["goods","order_*"]  
tables_ignore：数组，配置需要忽略的表，为空则是不忽略，例如: ["goods","order_*"]  
...
# mysql-schema-sync -help
  -conf string    配置文件名称
  -dest string    待同步的数据库，例如: username:password@(127.0.0.1:3306)/test_1
                  该项不为空时，忽略读入 -conf参数项
  -drop           是否对本地多出的字段和索引进行删除 默认否
  -mail_to string 替换配置文件里的email.to
  -source string  mysql 同步源,比如： username:password@(127.0.0.1:3306)/test_0
  -sync           是否将修改同步到数据库中去，默认否
  -tables string  待检查同步的数据库表，为空则是全部  比如: product_base,order_*
  -tables_ignore string 忽略检查的表名，比如：product_base,order_*
```
